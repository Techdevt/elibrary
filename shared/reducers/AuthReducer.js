    ...
    switch (action.type) {
        ...
        case 'AUTH_USER':
            ...
            return state.merge({
                ...
                user: {...action.res.data.user, address: action.res.data.user.address.map((address) => ({...address, editable: false, isActive: false}))},
        ...
        case 'EDIT_USER':
            ...
            return state.merge({
                isAuthenticated: true,
                user: Immutable.fromJS({...action.res.data.user, address: action.res.data.user.address.map((address) => Immutable.fromJS({...address, editable: true, isActive: true}))}),
                token: action.res.data.token,
        ...
        case 'CHANGE_SHOP_BANNER':
            return state.withMutations((state) => {
                state.setIn(['user', 'roles', 'merchant'], Immutable.fromJS(action.res.data))
                     .merge({
                        bannerLoading: false
                     });
            });
        ...
        case 'ONBOARD_MERCHANT':
            return state.withMutations((state) => {
                state.updateIn(['user', 'roles', 'merchant'], obj => {
        ...
        case 'EDIT_ADDRESS':
            const stateEdited = state.updateIn(['user', 'address'], arr => {
        ...
        case 'REVERT_ADDRESS':
            const oldAddress = Immutable.fromJS(action.address);
            return state.updateIn(['user', 'address'], arr => {
                return oldAddress;
            });
        case 'ADD_ADDRESS':
            return state.updateIn(['user', 'address'], arr => {
                return arr.push(Immutable.Map({}));
            });
        case 'TOGGLE_DELEGATE_ADDRESS_ACTIVE':
            return state.updateIn(['user', 'roles', 'merchant', 'delegates'], arr => {
                let activeDelegate = arr.get(action.dIndex).toJSON();
                let address = activeDelegate.address;
        ...
        case 'ADD_DELEGATE_ADDRESS':
            return state.updateIn(['user', 'roles', 'merchant', 'delegates'], arr => {
                let activeDelegate = arr.get(action.dIndex).toJSON();
                let address = activeDelegate.address.push({});
                activeDelegate.address = address;
        ...
        case 'RECEIVE_SOCKET':
            if(state.get('user')) {
                return state.update('user', obj => {
                    return obj.set('socketID', action.socketID);
                });
            }
            return state;
        default:
